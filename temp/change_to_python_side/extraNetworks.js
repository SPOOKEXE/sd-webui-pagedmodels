...
function setupExtraNetworksForTab(tabname) {
    ...
    this_tab.querySelectorAll(":scope > [id^='" + tabname + "_']").forEach(function(elem) {
        ...
        var applyPagination = function(page_number, items_per_page) {
            console.log("apply_pagination: ", tabname_full, page_number, items_per_page)
            page_number = Number(page_number);
            items_per_page = Number(items_per_page);
            var counter = 0;
            gradioApp().querySelectorAll('#' + tabname + '_extra_tabs div.card').forEach(function(elem) {
                var isAboveMin = counter >= ((page_number-1) * items_per_page);
                var isLessThanMax = counter <= (page_number * items_per_page);
                if (isAboveMin && isLessThanMax) {
                    elem.classList.remove("hidden");
                } else {
                    elem.classList.add("hidden");
                }
        ...
        extraNetworksApplyPagination[tabname_full] = applyPagination;
...
function applyExtraNetworkPagination(tabname_full, page_number, items_per_page) {
    console.log(tabname_full, page_number, items_per_page);
    var doFilter = function() {
        extraNetworksApplyPagination[tabname_full](page_number, items_per_page);
    };
    setTimeout(doFilter, 1);
}
...
var extraNetworksApplyPagination = {};
...
function extraNetworksControlSortOnClick(event, tabname, extra_networks_tabname) {
    ...
    applyExtraNetworkPagination(tabname + "_" + extra_networks_tabname, "1", "20");
}
